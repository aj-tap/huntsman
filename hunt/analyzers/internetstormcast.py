...
class Internetstormcast(AbstractAnalyzer):
    """
    ...
    def __init__(self, ioctype:str, observable: str, task_id: str):
        super().__init__(analyzer_name="internetstormcast")
        self.ioctype = ioctype
        self.observable = observable
        self.task_id = task_id

    def _get_ip_report(self):
        response = self._make_request(observable=self.observable, preffix_endpoint="ip/", endpoint=self.observable, suffix_endpoint="?json")
        if response:                       
            return response
        else:
